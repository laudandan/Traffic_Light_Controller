...
class Simulation:
    def __init__(self, Model, TrafficGen, sumo_cmd, max_steps, green_duration, yellow_duration, num_states, num_actions, incoming_roads, state_nr, ts):
        self._Model = Model
    ...
    def running(self):
        if self.index == 0 and self.flag_observations:
            self.current_state = self._get_state(self.state_nr)  # Pentru TL... 1 pentru TL1
        ...
        if self.index == 0 and self.flag_reward:
            ...
            self._reward_episode.append(self.reward)
            self.flag_observations = True
            self.flag_yellow = False
            self.flag_green = False
            self.flag_reward = False
    ...
    def _simulate(self, incoming_roads):
        """
